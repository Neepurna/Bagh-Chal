...
class BaghchalAI {
  constructor(difficulty = DIFFICULTY.MEDIUM) {
    ...
    // ENDGAME FIX 2-3: Per-move caches to avoid recalculating same positions
    this.threatCache = new Map();     // Caches isGoatThreatened() results
    this.chainCache = new Map();      // Caches countGoatChains() results
    this.moveCache = new Map();       // Caches getAllPossibleMoves() results
    this.boardStateHash = '';         // Current board state hash for cache validation
  }
  ...
  getBestMove(gameState, aiSide) {
    ...
    // ENDGAME FIX: Clear per-move caches at start of each move calculation
    this.threatCache.clear();
    this.chainCache.clear();
    this.moveCache.clear();
    this.boardStateHash = gameState.board.join(','); // Create board hash for cache validation
  ...
  isGoatThreatened(gameState, goatPos) {
    // ENDGAME FIX 2: Cache threatened goat checks to avoid O(n²) recalculation
  ...
  countGoatChains(gameState) {
    // ENDGAME FIX 3: Cache goat chain calculations to avoid recursive traversals
